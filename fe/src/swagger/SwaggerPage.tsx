...
export default function SwaggerPage() {
  ...
  return <Box component="main" id="main-content" sx={{ height: "100dvh", overflow: "hidden", bgcolor: "#f6f7fb", color: "#17212b" }}>
    <Box sx={{ height: "100%", overflowY: "auto", overscrollBehavior: "contain", scrollBehavior: "smooth", "&::-webkit-scrollbar": { width: 10 }, "&::-webkit-scrollbar-thumb": { bgcolor: "rgba(224,0,43,.42)", border: "3px solid #f6f7fb", borderRadius: 20 } }}>
      ...
      <Container maxWidth="xl" sx={{ py: { xs: 2, md: 4 }, pb: 7 }}><Stack spacing={3}>
        ...
        {spec && <Stack direction={{ xs: "column", lg: "row" }} spacing={3} sx={{ alignItems: "flex-start" }}>
          ...
          <Stack spacing={3} sx={{ minWidth: 0, flex: 1 }}>{Object.entries(grouped).map(([tag, operations]) => <Box component="section" id={anchorFor(tag)} key={tag} sx={{ scrollMarginTop: 84 }}><Stack direction="row" spacing={1.2} sx={{ alignItems: "center", mb: 1.5 }}><Box sx={{ width: 7, height: 30, borderRadius: 10, bgcolor: "primary.main" }} /><Box><Typography variant="h5" sx={{ fontWeight: 900 }}>{tag}</Typography><Typography variant="caption" color="text.secondary">{operations.length} endpoint{operations.length === 1 ? "" : "s"}</Typography></Box></Stack><Stack spacing={1.2}>{operations.map(endpoint => <EndpointCard endpoint={endpoint} key={`${endpoint.method}-${endpoint.path}`} />)}</Stack></Box>)}</Stack>
        </Stack>}
      </Stack></Container>
    </Box>
  </Box>;
