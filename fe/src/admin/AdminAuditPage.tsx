...
function describeAudit(t: TFunction, details: Record<string, unknown>) {
  ...
  if (created && typeof created === "object")
    return Object.entries(created as Record<string, unknown>)
      .map(([key, value]) => `${key}: ${String(value)}`)
      .join(" · ");
  const legacy = Object.entries(details)
    .map(([key, value]) => `${key}: ${String(value)}`)
    .join(" · ");
        ...
        <Stack spacing={1.25}>
          {logs.length === 0 && (
            <Paper sx={{ p: 3, borderRadius: 3 }}>
              <Typography color="text.secondary">{t("admin.noAuditLogs")}</Typography>
            </Paper>
          )}
          {logs.map((log) => (
            <Paper key={log.id} sx={{ p: 2, borderRadius: 2.5, borderLeft: "4px solid #e0002b" }}>
              <Stack
              ...
              >
                <Box>
                  ...
                  <Typography variant="body2" color="text.secondary">
                    {t("admin.administratorLabel")}: {log.actor || t("admin.deletedAccount")}
                    {log.target ? ` · ${t("admin.accountLabel")}: ${log.target}` : ""}
                  </Typography>
                  <Typography variant="body2" sx={{ mt: 0.75 }}>
                    {describeAudit(t, log.details)}
                  </Typography>
                </Box>
