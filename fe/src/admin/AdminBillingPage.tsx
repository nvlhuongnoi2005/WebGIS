    ...
    <>
      ...
      {!loading && !error && (
        <Stack spacing={2.5}>
          {selected && (
            <Paper
              sx={{ p: { xs: 2, sm: 3 }, borderRadius: 3, border: "1px solid rgba(224,0,43,.18)" }}
            >
              <Stack
                direction="row"
              ...
              <BillingCard
                ...
                range={range}
                onRangeChange={(value) => void changeRange(value)}
              />
            </Paper>
          )}
          <TableContainer
            component={Paper}
            sx={{ borderRadius: 3, border: "1px solid rgba(224,0,43,.14)" }}
          >
            <Table>
              <TableHead sx={{ bgcolor: "rgba(224,0,43,.08)" }}>
                <TableRow>
                  ...
                  <TableCell>{t("admin.used")}</TableCell>
                  <TableCell>{t("admin.quotaPeriod")}</TableCell>
                  <TableCell />
                </TableRow>
              </TableHead>
              <TableBody>
                {billings.map((billing) => (
                  <TableRow key={billing.userId} hover>
                    <TableCell>
                      <Typography sx={{ fontWeight: 700 }}>{billing.name}</Typography>
                    ...
                    <TableCell>
                      {billing.usedUnits} / {billing.limitUnits}
                    </TableCell>
                    <TableCell>
                      {formatPeriod(billing.periodStart, billing.periodEnd, i18n.language)}
                    </TableCell>
                    <TableCell align="right">
                      <Button size="small" onClick={() => void openBilling(billing)}>
                        {t("admin.viewQuota")}
                      </Button>
                    </TableCell>
                  </TableRow>
