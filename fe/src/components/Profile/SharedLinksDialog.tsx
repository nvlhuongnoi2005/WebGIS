...
export default function SharedLinksDialog({ open, onClose }: SharedLinksDialogProps) {
  ...
  const copyLink = async () => {
    try {
      ...
      setError(t("draw.shareCopyError"));
    }
  };
  ...
  return (
    ...
    >
        ...
        <Stack spacing={1.5}>
          ...
          {shareURL && (
            <Stack direction={{ xs: "column", sm: "row" }} spacing={1}>
              <TextField fullWidth size="small" label={t("draw.shareLink")} value={shareURL} slotProps={{ htmlInput: { readOnly: true } }} />
              <Button variant="outlined" startIcon={<Copy size={16} />} onClick={() => void copyLink()}>
                {copied ? t("draw.shareLinkCopied") : t("draw.copyShareLink")}
              </Button>
            </Stack>
          )}
          {loading ? <CircularProgress size={24} /> : shares.length === 0 ? (
            <Typography color="text.secondary">{t("draw.noActiveShares")}</Typography>
          ) : shares.map(share => (
            <Stack key={share.id} direction="row" spacing={1} sx={{ alignItems: "center", border: 1, borderColor: "divider", borderRadius: 2, p: 1 }}>
              <Box
                component="img"
                src={`data:image/svg+xml;charset=utf-8,${encodeURIComponent(share.preview_svg)}`}
                alt={t("draw.sharePreviewAlt")}
                sx={{ width: 112, height: 68, flex: "0 0 auto", objectFit: "contain", border: 1, borderColor: "divider", borderRadius: 1 }}
              />
              <Stack spacing={0.25} sx={{ minWidth: 0, flex: 1 }}>
                <Typography variant="body2" sx={{ fontWeight: 700 }}>{t("draw.sharePreviewCount", { count: share.feature_count })}</Typography>
                <Typography variant="caption" color="text.secondary">{t("profile.linkCreated", { date: new Date(share.created_at).toLocaleDateString() })}</Typography>
              </Stack>
              <Stack spacing={0.5}>
                <Button size="small" startIcon={openingID === share.id ? <CircularProgress size={14} /> : <Link2 size={15} />} onClick={() => void showShareLink(share)} disabled={openingID === share.id}>
                  {t("profile.showLink")}
                </Button>
                <Button size="small" color="error" startIcon={revokingID === share.id ? <CircularProgress size={14} /> : <Trash2 size={15} />} onClick={() => void revokeShare(share)} disabled={revokingID === share.id}>
                  {t("draw.revokeShare")}
                </Button>
              </Stack>
            </Stack>
      ...
      <DialogActions sx={{ px: 3, pb: 2.5 }}><Button onClick={onClose}>{t("profile.close")}</Button></DialogActions>
    </Dialog>
  );
}
