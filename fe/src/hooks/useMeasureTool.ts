...
import { isSelfIntersectingPolygon } from "../tools/measure/MeasureTool";
...
export function useMeasureTool({
  ...
  const commitMeasurePoints = useCallback(
    (nextPoints: Coordinate[]) => {
      setUndoStack((history) => [...history, measurePoints]);
  ...
  const undoMeasure = useCallback(() => {
    ...
    setMeasureError(null);
    setIsAreaComplete(false);
    setRedoStack((history) => [...history, measurePoints]);
  ...
  const changeMeasureMode = useCallback(
    (mode: MeasureMode) => {
      if (mode === measureMode) {
        return;
      }
  ...
  useEffect(() => {
    ...
    mapInstance.on("mouseenter", "measure-points-layer", handleMouseEnter);
    mapInstance.on("mouseleave", "measure-points-layer", handleMouseLeave);

    return () => {
      canvas.style.cursor = "";
      mapInstance.off("mouseenter", "measure-points-layer", handleMouseEnter);
      mapInstance.off("mouseleave", "measure-points-layer", handleMouseLeave);
    };
  }, [activeTool, map, mapLoaded]);

  useEffect(() => {
    const handleKeyDown = (event: KeyboardEvent) => {
      if (event.key === "Escape" && activeTool === "measure") {
        resetMeasure();
        setActiveTool(null);
      ...
      if (activeTool !== "measure" || !(event.ctrlKey || event.metaKey)) {
        return;
      }
  ...
  }, [activeTool, redoMeasure, resetMeasure, setActiveTool, undoMeasure]);
  ...
  useEffect(() => {
    ...
    const handleMapClick = (event: MapMouseEvent) => {
      ...
      const features = mapInstance.getLayer("measure-points-layer")
        ? mapInstance.queryRenderedFeatures(bbox, {
      ...
      if (features.length > 0) {
        ...
        if (typeof index === "number") {
          if (measureMode === "area") {
            ...
            const polygonCandidate = measurePoints.slice(index);

            if (polygonCandidate.length >= 3) {
              if (isSelfIntersectingPolygon(polygonCandidate)) {
                setMeasureError("measure.errorSelfIntersect");
            ...
            setMeasureError(null);
            commitMeasurePoints(measurePoints.filter((_, pointIndex) => pointIndex !== index));
      ...
      if (measureMode === "area" && isSelfIntersectingPolygon(nextPoints)) {
        setMeasureError("measure.errorSelfIntersect");
  ...
  }, [activeTool, commitMeasurePoints, map, mapLoaded, isAreaComplete, measureMode, measurePoints]);
