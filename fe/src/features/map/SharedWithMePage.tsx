...
export default function SharedWithMePage() {
  ...
  useEffect(() => {
    let active = true;
    void listReceivedGeoJSONShares()
      .then((result) => {
  ...
  return (
    ...
    >
      <AppBar position="static" color="inherit" elevation={1}>
        <Toolbar sx={{ gap: 1 }}>
          <Button onClick={() => navigate("/map")} startIcon={<ArrowLeft size={18} />}>
            {t("draw.backToMap")}
          </Button>
          ...
          <List sx={{ bgcolor: "background.paper", borderRadius: 2, boxShadow: 1 }}>
            {shares.map((share) => (
              <ListItem key={share.id} disablePadding divider>
                <ListItemButton
                  onClick={() => navigate(`/shared-with-me/${encodeURIComponent(share.id)}`)}
                >
                  <ListItemText
