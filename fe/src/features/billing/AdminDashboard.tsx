    ...
    <PageShell title={t("admin.title")}>
      ...
      {metrics && <>
        <Stack direction={{ xs: "column", sm: "row" }} spacing={2}>
          <MetricCard icon={<Users size={23} />} label={t("admin.totalAccounts")} value={metrics.totalAccounts} accent="#2563c8" tint="rgba(37, 99, 200, 0.13)" />
          <MetricCard icon={<Activity size={23} />} label={t("admin.onlineUsers")} value={metrics.onlineUsers} accent="#008b7f" tint="rgba(0, 139, 127, 0.13)" />
        </Stack>
        <Paper sx={{ p: { xs: 2, sm: 3 }, borderRadius: 3, border: "1px solid rgba(132, 64, 186, 0.17)", background: "linear-gradient(135deg, rgba(132, 64, 186, 0.12), rgba(38, 103, 201, 0.08) 52%, transparent)" }}>
        ...
        <Box>
          ...
          <TableContainer component={Paper} sx={{ borderRadius: 3, overflow: "hidden", border: "1px solid rgba(38, 103, 201, 0.14)" }}>
            <Table size="small" aria-label={t("admin.billingDetails")}>
              <TableHead sx={{ background: "linear-gradient(90deg, rgba(38, 103, 201, 0.14), rgba(132, 64, 186, 0.10), rgba(0, 139, 127, 0.12))" }}><TableRow><TableCell>{t("admin.account")}</TableCell><TableCell>{t("billing.plan")}</TableCell><TableCell>{t("billing.monthlyUsage")}</TableCell><TableCell>{t("billing.period")}</TableCell></TableRow></TableHead>
              <TableBody>
        ...
        <Box>
          ...
          <TableContainer component={Paper} sx={{ borderRadius: 3, overflow: "hidden", border: "1px solid rgba(0, 139, 127, 0.16)" }}>
            <Table size="small"><TableHead sx={{ background: "linear-gradient(90deg, rgba(0, 139, 127, 0.13), rgba(229, 106, 0, 0.10), rgba(224, 0, 43, 0.10))" }}><TableRow><TableCell>{t("admin.account")}</TableCell><TableCell>{t("admin.role")}</TableCell><TableCell>{t("admin.status")}</TableCell><TableCell>{t("billing.plan")}</TableCell><TableCell>{t("admin.quota")}</TableCell><TableCell>{t("admin.online")}</TableCell></TableRow></TableHead>
...
function MetricCard({ icon, label, value, accent, tint }: { icon: ReactNode; label: string; value: number; accent: string; tint: string }) {
  return <Paper sx={{ p: 2.5, borderRadius: 3, flex: 1, border: `1px solid ${tint}`, background: `linear-gradient(135deg, ${tint}, transparent 70%)` }}><Stack direction="row" spacing={1.5} sx={{ alignItems: "center", color: accent }}><Box sx={{ display: "grid", placeItems: "center", width: 42, height: 42, borderRadius: 2.5, bgcolor: tint }}>{icon}</Box><Box><Typography variant="body2" color="text.secondary">{label}</Typography><Typography variant="h4" color="text.primary" sx={{ fontWeight: 750 }}>{value}</Typography></Box></Stack></Paper>;
}

function AgeBar({ label, value, max, index }: { label: string; value: number; max: number; index: number }) {
  const color = BAR_COLORS[index % BAR_COLORS.length];
  return <Stack spacing={0.75} sx={{ alignItems: "center", flex: 1, minWidth: 28, height: "100%", justifyContent: "end" }}><Typography variant="caption" sx={{ fontWeight: 700, color }}>{value}</Typography><Box sx={{ width: "100%", maxWidth: 48, minHeight: value ? 4 : 0, height: `${value / max * 128}px`, borderRadius: "8px 8px 2px 2px", background: `linear-gradient(180deg, ${color}, ${color}88)`, boxShadow: `0 5px 10px ${color}33` }} /><Typography variant="caption" color="text.secondary">{label}</Typography></Stack>;
}

const BAR_COLORS = ["#e0002b", "#2563c8", "#008b7f", "#8540ba", "#d16a00", "#c23e8a"];
const AUDIT_COLORS = ["#e0002b", "#2563c8", "#008b7f", "#d16a00", "#8540ba"];
