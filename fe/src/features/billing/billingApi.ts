...
export async function fetchAdminUsers() {
  return (await getJson<{ users: AdminUser[] }>("/api/admin/users")).users;
}
export async function updateAdminUser(
  id: string,
  update: Partial<Pick<AdminUser, "name" | "email" | "role" | "status" | "plan" | "limitUnits">>
) {
...
export async function resetAdminUserPassword(id: string, newPassword: string) {
  const response = await authFetch(`/api/admin/users/${encodeURIComponent(id)}/reset-password`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
export async function deleteAdminUser(id: string) {
  const response = await authFetch(`/api/admin/users/${encodeURIComponent(id)}`, {
    method: "DELETE",
  });
  if (!response.ok) throw new Error(`Request failed (${response.status})`);
}
export async function fetchAdminAudit() {
  return (await getJson<{ logs: AuditLog[] }>("/api/admin/audit")).logs;
}
