...
const userColumns = `id, email, password_hash, status, auth_version, scopes, plan, role, full_name, date_of_birth::text, phone, organization, avatar_url, must_change_password`

func scanUser(row pgx.Row) (User, error) {
	var user User
	err := row.Scan(&user.ID, &user.Email, &user.PasswordHash, &user.Status, &user.AuthVersion, &user.Scopes, &user.Plan, &user.Role, &user.Name, &user.DateOfBirth, &user.Phone, &user.Organization, &user.AvatarURL, &user.MustChangePassword)
	return user, err
}
...
func publicUser(user User) map[string]any {
	result := map[string]any{"id": user.ID, "email": user.Email, "plan": user.Plan, "role": user.Role, "scopes": user.Scopes, "mustChangePassword": user.MustChangePassword}
	if user.Name != nil {
