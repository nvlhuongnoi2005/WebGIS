...
func openAPISpec() map[string]any {
	...
	return map[string]any{
		...
		"tags":    []map[string]string{{"name": "Authentication"}, {"name": "Map"}, {"name": "Billing"}, {"name": "Administration"}},
		"components": map[string]any{
			...
			"schemas": map[string]any{
				...
				"Billing":     map[string]any{"type": "object", "properties": map[string]any{"userId": map[string]string{"type": "string"}, "plan": map[string]string{"type": "string"}, "limitUnits": map[string]string{"type": "integer"}, "usedUnits": map[string]string{"type": "integer"}, "dailyRequests": map[string]any{"type": "array"}}},
			},
		},
		"paths": map[string]any{
			...
			"/api/suggestions":            map[string]any{"get": queryOperation("Map", "Gợi ý địa điểm", "q", "Chuỗi tìm kiếm (2–120 ký tự)", nil)},
			"/api/nominatim/search":       map[string]any{"get": queryOperation("Map", "Tìm kiếm Nominatim qua gateway", "q", "Địa điểm cần tìm", nil)},
			...
			"/api/tiles/{path}":           map[string]any{"get": securedOperation("Map", "Proxy tile server", nil, map[string]any{"parameters": []map[string]any{{"name": "path", "in": "path", "required": true, "schema": map[string]string{"type": "string"}}}})},
...
func queryOperation(tag, summary, name, description string, body map[string]any) map[string]any {
	value := operation(tag, summary, body, nil)
